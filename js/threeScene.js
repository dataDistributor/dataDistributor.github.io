...
if (!containerElement || prefersReducedMotion) {
  if (!containerElement) {
    console.warn('No scene container found for Three.js background');
  }
  if (prefersReducedMotion) {
    console.log('Three.js background disabled due to prefers-reduced-motion');
  }
} else {
  // Scene and camera
  ...
  // Lights
  const ambientLight = new THREE.AmbientLight(0x8899ff, 0.55);
  ...
  const rimLight = new THREE.DirectionalLight(0x4f9dff, 0.7);
  ...
  const orbitCount = isMobile ? 220 : 420;
  const streamCount = isMobile ? 70 : 130;
  ...
  for (let i = 0; i < orbitCount; i += 1) {
    ...
    const verticalOffset = (Math.random() - 0.5) * 22;
    const speed =
      (0.06 + Math.random() * 0.18) * (Math.random() > 0.5 ? 1 : -1);
    const phase = Math.random() * Math.PI * 2;

    const hue = 0.55 + (radius / 80) * 0.1;
    orbitColor.setHSL(hue, 0.7, 0.55);
  ...
  for (let i = 0; i < streamCount; i += 1) {
    ...
    const travelDuration = 4.5 + Math.random() * 4.0;
  ...
  window.addEventListener('scroll', () => {
    ...
    interactionState.cameraZ = 80 - clamped * 14;
  });
  ...
  function renderFrame() {
    ...
    coreMesh.rotation.x = Math.sin(elapsed * 0.28) * 0.18;

    // Camera parallax and subtle dolly
    const parallaxStrengthX = 10;
    const parallaxStrengthY = 9;
    ...
    for (let i = 0; i < orbitCount; i += 1) {
      ...
      const wobble =
        Math.sin(elapsed * 0.7 + orbitInfo.phase) * 2.0 +
        Math.sin(elapsed * 1.1 + orbitInfo.phase * 1.5) * 0.8;
      ...
      orbitEuler.set(
        Math.sin(elapsed * 0.5 + orbitInfo.phase) * 0.45,
        angle,
        0
      );
    ...
    for (let i = 0; i < streamCount; i += 1) {
      ...
      const baseScale = 1 + easedTime * 0.9;
  ...
  console.log('Three.js data-flow background initialised (reduced motion friendly)');
}
